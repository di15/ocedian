...
bool FindTexture(unsigned int &texture, LPSTR filepath)
{
	...
	for(int i=0; i<TEXTURES; i++)
		if(g_texture[i].loaded && stricmp(g_texture[i].filepath, filepath) == 0)
		{
...
void TextureLoaded(unsigned int texture, char* filepath, int id)
{
	for(int i=0; i<strlen(filepath); i++)
		if(filepath[i] == '/')
			filepath[i] = '\\';
	...
	strcpy(g_texture[i].filepath, filepath);
...
bool CreateTexture(unsigned int &texture, LPSTR strFileName, int id)
{
	if(!strFileName) 
		return false;
